...
#' queue.
#' @param label A character string containing a unique label for the
#' permission.
#' @param principal A character vector containing the AWS account number of the
#' principal who will be given permission. Principals and actions must be
#' specified one-to-one or one-to-many.
#' @param action A character vector containing an SQS permission. Valid values
#' include: \dQuote{*}, \dQuote{SendMessage}, \dQuote{ReceiveMessage},
#' \dQuote{DeleteMessage}, \dQuote{ChangeMessageVisibility},
#' \dQuote{GetQueueAttributes}, \dQuote{GetQueueUrl}.
#' @param ... Additional arguments passed to \code{\link{sqsHTTP}}.
#' @return A list.
#' @author Thomas J. Leeper
#' @seealso \code{link{create_queue}} \code{\link{get_queue_attrs}}
